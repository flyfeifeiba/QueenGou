...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
     ...
     */
    //segment的偏移量，通过segmentShift可以找到在哪个segement  和ConcurrentHashMap类似
     ...
     */
    /** key的引用类型（strong、weak、soft） */
    final Strength keyStrength;

    /**
     * Strategy for referencing values.
     */
    /** value的引用类型（strong、weak、soft） */
    ...
    LocalCache(
        ...
        int segmentShift = 0;
        int segmentCount = 1;
        //找一个刚刚大于或等于concurrencyLevel的Segment数
        ...
        //计算每一个Segment中的容量的值，刚刚大于等于initialCapacity/segmentCount
        int segmentCapacity = initialCapacity / segmentCount;
        if (segmentCapacity * segmentCount < initialCapacity) {
            ++segmentCapacity;
        }

        //刚刚>=segmentCapacity&&是2的几次方的数
        int segmentSize = 1;
        while (segmentSize < segmentCapacity) {
            segmentSize <<= 1;
        }

        //由于没有设置maxWeight，所以其值为-1-->evictsBySize()返回false
        if (evictsBySize()) {
    ...
    static class Segment<K, V> extends ReentrantLock {
         ...
         */
        @GuardedBy("this")
        //map中当前元素写队列，队列元素根据write time进行排序，每write一个元素就将该元素加在队列尾部
         ...
         */
        @GuardedBy("this")
        //map中当前元素读队列，队列元素根据write time进行排序，每write一个元素就将该元素加在队列尾部
        ...
        Segment(
            ...
            //判断是否为强引用，若强引用，则keyReferenceQueue为空
        ...
        ReferenceEntry<K, V> getEntry(Object key, int hash) {
            //通过循环ReferenceEntey链找到相应的缓存，没有就返回空
        ...
        V put(K key, int hash, V value, boolean onlyIfAbsent) {
            ...
            try {
                ...
                //通过二次hash的值定位到那个ReferenceEntry链
                AtomicReferenceArray<ReferenceEntry<K, V>> table = this.table;
                int index = hash & (table.length() - 1);
                ReferenceEntry<K, V> first = table.get(index);

                // Look for an existing entry.
                //遍历这个ReferenceEntry链表
    ...
    static class LocalLoadingCache<K, V> extends LocalManualCache<K, V>
        ...
        LocalLoadingCache(
            ...
            //3）最后调用父类LocalManualCache的私有构造器将创建出来的LocalCache实例赋给LocalCache的类变量，完成初始化。
